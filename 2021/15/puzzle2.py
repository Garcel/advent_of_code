...
class CavesMap(object):
    ...
    def __init__(self, weights: [[int]]):
        tile_weights = list(map(list, zip(*weights)))

        self.weights = []
        self.explored = []
        for x in range(0, len(tile_weights) * 5):
            self.weights.append([])
            self.explored.append([])

            for y in range(0, len(tile_weights[0]) * 5):
                tile_x = math.trunc(x / len(tile_weights))
                tile_y = math.trunc(y / len(tile_weights[0]))

                tile_weight = tile_weights[x % len(tile_weights)][y % len(tile_weights[0])]
                weight = (tile_weight + tile_x + tile_y)
                if weight > 9:
                    weight = weight % 9

                self.weights[x].append(weight)
    ...
    def find_path_with_lowest_risk(self) -> int:
        ...
        while self.visited:
            ...
            self.explored[current[0]][current[1]] = True

            current_cost = self.cost[current[0]][current[1]]
            siblings = self._get_siblings(current)

            for sibling in siblings:
                if self.explored[sibling[0]][sibling[1]] or sibling in self.visited:
                    continue
